  ...
  <form id="goods_class_form" name="goodsClassForm" enctype="multipart/form-data" method="post">
    ...
    <div class="ncap-form-default">
      <dl class="row">
        ...
        <dl class="row">
            <dt class="tit">
                <label for="">二级分类图片</label>
            </dt>
            <dd class="opt">
                <div class="input-file-show"><span class="show"> <a class="nyroModal" rel="gal" href="<?php echo UPLOAD_SITE_URL.DS.ATTACH_COMMON.'/'. $output['class_array']['gc_image'];?>"> <i class="fa fa-picture-o" onMouseOver="toolTip('<img src=<?php echo UPLOAD_SITE_URL.DS.ATTACH_COMMON.'/'.$output['class_array']['gc_image'];?>>')" onMouseOut="toolTip()"></i></a> </span> <span class="type-file-box">
            <input name="gc_image" type="file" class="type-file-file" id="gc_image" size="30" hidefocus="true">
            </span></div>
                <span class="err"></span>
        ...
        <dl class="row">
            <dt class="tit">
                <label for="">绑定的规格分类</label>
            </dt>
            <?php if(is_array($output['spec_list'])&&!empty($output['spec_list'])){?>
            <dd class="opt">
                <?php foreach ($output['spec_list'] as $k=>$v){?>
                <label for="t_associated">
                    <input class="checkbox" type="checkbox" name="gc_spec_id[]" value="<?php echo $v['sp_id']?>" id="gc_spec_id" <?php if(in_array($v['sp_id'],$output['gc_spec_id'])){?>checked <?php }?> />
                    <?php echo $v['sp_name'];?>
                </label>
                <?php }?>
            </dd>
...
<script>
    $(function(){
        ...
        $(textButton).insertBefore("#gc_image");
        $("#gc_image").change(function(){
            $("#textfield1").val($("#gc_image").val());
        });
//按钮先执行验证再提交表单
        $("#submitBtn").click(function(){
            if($("#goods_class_form").valid()){
