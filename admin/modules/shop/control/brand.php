    ...
    public function brand_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
				...
				// 上传品牌大图
            	if ($_FILES['brand_filebgpic']['name'] != ''){
					$upload = new UploadFile();
					$upload->set('default_dir',ATTACH_BRAND);

					$result = $upload->upfile('brand_filebgpic');
					if ($result){
						$_POST['brand_bgpic'] = $upload->file_name;
					}else {
    ...
    public function brand_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
					if (!empty($_POST['brand_bgpic']) && !empty($brand_info['brand_bgpic'])){
                        @unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.'/bgpic'.DS.$brand_info['brand_bgpic']);
                    }
