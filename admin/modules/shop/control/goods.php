...
class goodsControl extends SystemControl{
    private $links = array(
        ...
        array('url'=>'act=goods&op=product_goods','text'=>'报单产品'),
    ...
    public function get_xmlOp() {
        ...
        switch ($_GET['type']) { 
            ...
            case 'waitverify':
                ...
                //报单产品
            case 'product':
                $goods_list = $model_goods->getGoodsCommonProductList($condition, '*', $page, $order);
                break;
                // 全部商品
            default:
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            switch ($_GET['type']) {
                ...
                //报单产品
                case 'product':
                    $count = $model_goods->getGoodsCommonProductList($condition);
                    break;
                    // 全部商品
                default:
        ...
        switch ($_GET['type']) {
            ...
            //报单产品
            case 'product':
                $goods_list = $model_goods->getGoodsCommonProductList($condition, '*', null, $order, $limit);
                break;
            // 全部商品
            default:
...
//                'virtual_invalid_refund' => '允许退款'
     ...
     * 报单产品
     */
    public function product_goodsOp(){
        Tpl::output('type', 'product');
        Tpl::output('top_link',$this->sublink($this->links,'product_goods'));
