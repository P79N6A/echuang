	...
	public function goods_class_addOp() {
		...
		if (chksubmit()) {
			...
			if ($error != '') {
				...
				$insert_array['gc_image'] ="";
				$result_id = $model_class->addGoodsClass($insert_array);
				if ($_FILES['gc_image']['tmp_name'] != ''){
					$upload = new UploadFile();
					$upload->set('default_dir',ATTACH_COMMON);
					$upload->file_name=make_sn($result_id).".png";
                    $update_array['gc_image'] =$upload->file_name;
					$result = $upload->upfile('gc_image');
				}
				$where['gc_id']=$result_id;
...
//				array("input" => $_POST["commis_rate"], "require" => "true", 'validator' => 'range', 'max' => 100, 'min' => 0, "message" => $lang['goods_class_add_commis_rate_error']),
			...
			$update_array['gc_spec_id'] = implode(',',$_POST['gc_spec_id']);
			$update_array['type_id'] = intval($_POST['t_id']);
			$update_array['type_name'] = trim($_POST['t_name']);
			$update_array['commis_rate'] = intval(10);
			...
			if ($_FILES['gc_image']['tmp_name'] != ''){
				$upload = new UploadFile();
				$upload->set('default_dir',ATTACH_COMMON);
				$upload->file_name=make_sn($_POST['gc_id']).".png";
				$result = $upload->upfile('gc_image');
				if ($result){
					$_POST['gc_image'] = $upload->file_name;
				}else {
					showMessage($upload->error);
				}
			}
			if (!empty($_POST['gc_image'])){
				$update_array['gc_image'] = $_POST['gc_image'];
			}
		...
		Tpl::output('gc_list', $gc_list);
		//分类下绑定的规格
		$spec_list=Model('spec')->specList('');
		Tpl::output('spec_list', $spec_list);

		$gc_spec_info=Model('goods_class')->getGoodsClassInfoById($_GET['gc_id']);
		$gc_spec_id=explode(',',$gc_spec_info['gc_spec_id']);
		Tpl::output('gc_spec_id', $gc_spec_id);
