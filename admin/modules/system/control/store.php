...
class storeControl extends SystemControl{
    ...
    private $_links = array(
		...
		array('url'=>'act=store&op=distribution_setting','text'=>'分销规则')
    );
     ...
     * 分销规则
     */
    public function distribution_settingOp(){
    	$model_setting = Model('setting');
		if (chksubmit()){
			$update_array = array();
			$update_array['hao_invite2'] = $_POST['hao_invite2'];
			$update_array['hao_invite3'] = $_POST['hao_invite3'];
        ...
        Tpl::output('top_link',$this->sublink($this->_links,'distribution_setting'));
		Tpl::setDirquna('system');
        Tpl::showpage('hao.distribution_setting');
    }
