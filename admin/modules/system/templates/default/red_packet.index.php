...
<link href="<?php echo ADMIN_SITE_URL?>/templates/default/css/base.css" rel="stylesheet" type="text/css">
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="index.php?act=red_packet&op=new" ><span>新增</span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="get" name="formSearch">
    <input type="hidden" name="act" value="red_packet">
    ...
    <table class="table tb-type2">
      ...
      <tbody id="treet1">
        ...
        <tr class="hover edit row">
		  <?php
             //$used = Model()->table('red_packet_rec')->where(array('packet_id'=>$v['id'],'is_use'=>1))->select();
			 //$no_use = Model()->table('red_packet_rec')->where(array('packet_id'=>$v['id'],'is_use'=>3))->select();
		  ?>
          <td><a href="<?php echo WAP_SITE_URL; ?>/tmpl/red_packet.html?id=<?php echo $v['id']; ?>" target="_blank"><?php echo $v['packet_name'];?></a></td>
		<td><a href="<?php echo WAP_SITE_URL; ?>/tmpl/red_packet.html?id=<?php echo $v['id']; ?>" target="_blank"><?php echo WAP_SITE_URL; ?>/tmpl/red_packet.html?id=<?php echo $v['id']; ?></a></td>
          <td class="align-center"><?php echo $v['packet_amount'];?></td>
		  <td class="align-center"><?php echo $v['packet_number'];?></td>
		  <td class="align-center"><a href="index.php?act=red_packet&op=uselist&id=<?php echo $v['id']; ?>"><?php echo $v['packet_numbered'];?></a></td>
		  <!--td class="align-center"><a href="index.php?act=red_packet&op=uselist&id=<?php echo $v['id']; ?>"><?php echo count($used);?></a></td>
		  <td class="align-center"><a href="index.php?act=red_packet&op=uselist&id=<?php echo $v['id']; ?>"><?php echo count($no_use);?></a></td-->
          ...
          <td class="align-center">
		    <a href="index.php?act=red_packet&op=list&id=<?php echo $v['id']; ?>">查看</a> | 
          	<a href="index.php?act=red_packet&op=edit&id=<?php echo $v['id']; ?>">编辑</a> |
			<?php if($v['state']==1){ ?>
			<a href="javascript:if(confirm('确定关闭?'))window.location.href='index.php?act=red_packet&op=state&id=<?php echo $v['id']; ?>&type=close'">关闭</a>
			<?php }else{ ?>
			<a href="javascript:if(confirm('确定开启?'))window.location.href='index.php?act=red_packet&op=state&id=<?php echo $v['id']; ?>&type=open'">开启</a>
			<?php } ?>
			|
			<?php if($v['state']==1 && $v['start_time']<time() && $v['end_time']>time()){ ?>
			<a href="javascript:;"><span style="color:#666;">删除</span></a>
			<?php }else{ ?>
			<a href="javascript:if(confirm('确定删除?'))window.location.href='index.php?act=red_packet&op=del&id=<?php echo $v['id']; ?>'">删除</a>
			<?php } ?>
          </td>
        </tr>
        <?php } ?>
