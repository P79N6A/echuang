...
<script type="text/javascript">
    $(function(){
        $("#flexigrid").flexigrid({
            url: 'index.php?act=city_owner_rebate&op=get_xml',
            colModel : [
                {display: '订单号', name : 'order_number', width : 150, sortable : true, align: 'center'},
                {display: '红包单号', name : 'envelopes_order_number', width : 150, sortable : true, align: 'center'},
                {display: '用户手机', name : 'member_mobile', width : 150, sortable : true, align: 'center'},
                {display: '金额', name : 'money', width : 50, sortable : true, align: 'center'},
                {display: '分润时间', name : 'addtime', width : 180, sortable : true, align: 'center'},
                {display: '备注', name : 'desc', width : 220, sortable : true, align: 'center'},
            ],
            buttons : [
    ...
    $('#ncsubmit').click(function() {
        $("#flexigrid").flexOptions({ url: 'index.php?act=city_owner_rebate&op=get_xml&' + $("#formSearch").serialize(), query: '', qtype: '' }).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function() {
        $("#flexigrid").flexOptions({ url: 'index.php?act=city_owner_rebate&op=get_xml' }).flexReload();
        $("#formSearch")[0].reset();
    });
