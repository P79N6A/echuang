                        ...
                        <dd>
                            <label>
                                <select class="s-select" name="keyword_type">
                                    ...
                                    <option value="wo_member_mobile">用户手机</option>
...
<script type="text/javascript">
    $(function() {
        $("#flexigrid").flexigrid({
            url: 'index.php?act=member_rebate&op=get_xml',
            colModel : [
                {display: '订单号', name: 'order_number', width : 200, sortable : false, align: 'center'},
                {display: '红包单号', name: 'red_envelopes_order_number', width : 200, sortable : false, align: 'center'},
                {display: '用户手机', name: 'wo_member_mobile', width : 150, sortable : false, align: 'center'},
                {display: '金额', name: 'money', width : 100, sortable : false, align: 'center'},
                {display: '分润时间', name: 'addtime', width : 200, sortable : false, align: 'center'},
                {display: '备注', name: 'we_member_mobile', width : 200, sortable : false, align: 'center'}
            ],
            buttons : [
            ...
            searchitems: [
                ...
                {display: '用户手机', name: 'wo_member_mobile'},
    ...
    function fg_operation(name, bDiv) {
        if (name == 'xls') {
            ...
            window.location.href = 'index.php?act=member_rebate&op=export_step1';
        }
    }
    // 高级搜索提交
    $('#ncsubmit').click(function() {
        $("#flexigrid").flexOptions({ url: 'index.php?act=member_rebate&op=get_xml&' + $("#formSearch").serialize(), query: '', qtype: '' }).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function() {
        $("#flexigrid").flexOptions({ url: 'index.php?act=member_rebate&op=get_xml' }).flexReload();
        $("#formSearch")[0].reset();
    });
