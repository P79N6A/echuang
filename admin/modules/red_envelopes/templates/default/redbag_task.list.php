...
<div class="page">
    ...
    <script type="text/javascript">
        $(function(){
            $("#flexigrid").flexigrid({
                url: 'index.php?act=redbag_task&op=get_xml',
                colModel : [
                    {display: '操作', name : 'operation', width : 200, sortable : true, align: 'center'},
                    {display: '任务标题', name : 'title', width : 100, sortable : true, align: 'center'},
                    {display: '任务总数', name : 'task_number', width : 100, sortable : true, align: 'center'},
                    {display: '已领数', name : 'quantity_received', width : 100, sortable : true, align: 'center'},
                    {display: '剩余数', name : 'surplus_quantity', width : 150, sortable : true, align: 'center'},
                    {display: '任务内容', name : 'task_content', width : 200, sortable : true, align: 'center'},
                    {display: '任务图片', name : 'picture', width : 50, sortable : true, align: 'center'},
                    {display: '对比图片', name : 'contrast_picture', width : 100, sortable : true, align: 'center'},
                    {display: '总金额', name : 'task_total_sum', width : 100, sortable : true, align: 'center'},
                    {display: '任务订单号', name : 'task_order', width : 150, sortable : true, align: 'center'},
                ...
                buttons : [
                    ...
                    {display: '<a href="index.php?act=redbag_task&op=redbag_task_add"><i class="fa fa-file-excel-o"></i>添加</a>'}
                ],
                searchitems : [
        ...
        $('#ncsubmit').click(function() {
            $("#flexigrid").flexOptions({ url: 'index.php?act=redbag_task&op=get_xml&' + $("#formSearch").serialize(), query: '', qtype: '' }).flexReload();
        });

        // 高级搜索重置
        $('#ncreset').click(function() {
            $("#flexigrid").flexOptions({ url: 'index.php?act=redbag_task&op=get_xml' }).flexReload();
            $("#formSearch")[0].reset();
        });
