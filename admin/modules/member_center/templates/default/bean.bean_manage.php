            ...
            <div id="searchCon" class="content">
                <div class="layout-box">
                    ...
                    <dl>
                        <dt>金豆类型</dt>
                        <label>
                            <select name="bean_type" class="s-select">
                                <option value="">-请选择-</option>
                                <option value="rebate">分享奖励</option>
                                <option value="consume">消费</option>
                                <option value="withdraw">提现</option>
                                <option value="admin">redbag</option>
                                <option value="donate">转赠</option>
                                <option value="present">赠送</option>
                                <option value="order_pay">订单支付</option>
                                <option value="dividend">商城赠送</option>
                                <option value="promotion">推广奖</option>
                                <option value="extend">扩展奖</option>
...
$(function() {
    $("#flexigrid").flexigrid({
        url: 'index.php?act=bean&op=get_xml',
        colModel : [
            ...
            {display: '会员姓名', name: 'lg_member_name', width : 100, sortable : false, align: 'center'},
            {display: '单号', name: 'lg_sn', width : 160, sortable : false, align: 'center'},
            {display: '类型', name: 'lg_type', width : 100, sortable : false, align: 'center'},
            {display: '金额', name: 'lg_amount', width : 100, sortable : false, align: 'center'},
            {display: '调整后金额', name: 'lg_bean_amount', width : 160, sortable : false, align: 'center'},
            {display: '时间', name: 'lg_addtime', width : 160, sortable : false, align: 'center'},
            {display: '备注', name: 'lg_desc', width : 260, sortable : false, align: 'center'},
        ],
        buttons : [
        ...
        searchitems: [
            ...
            {display: '会员姓名', name: 'lg_member_name'},
            {display: '单号', name: 'lg_sn'},
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?act=bonus&op=member_add';
    }
    if (name == 'xls') {
...
$('#ncsubmit').click(function() {
    $("#flexigrid").flexOptions({ url: 'index.php?act=bean&op=get_xml&' + $("#formSearch").serialize(), query: '', qtype: '' }).flexReload();
});

// 高级搜索重置
$('#ncreset').click(function() {
    $("#flexigrid").flexOptions({ url: 'index.php?act=bean&op=get_xml' }).flexReload();
    $("#formSearch")[0].reset();
});
