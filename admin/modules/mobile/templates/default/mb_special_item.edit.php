    ...
    <div class="fixed-bar">
        <div class="item-title">
            <?php if ($output['item_info']['special_id'] > 0) { ?>
                <a id="btn_back"
                   href="<?php echo urlAdminMobile('mb_special', 'special_edit', array('special_id' => $output['item_info']['special_id'])); ?>"
                   class="back" title="返回上一级"><i class="fa fa-arrow-circle-o-left"></i></a>
            <?php } else { ?>
                <a id="btn_back" href="<?php echo urlAdminMobile('mb_special', 'index_edit'); ?>" class="back"
                   title="返回上一级"><i class="fa fa-arrow-circle-o-left"></i></a>
            <?php } ?>
            <div class="subject">
...
<div id="dialog_item_edit_image" style="display:none;">
    ...
    <form id="form_image" action="">
        <div class="ncap-form-default">
            ...
            <input id="btn_upload_image1" type="file" name="special_image" class="type-file-file" size="30"
                   hidefocus="true">
            </span>
                    </div>
            ...
            <dl class="row">
                <dt class="tit">导航文字：</dt>
                <dd class="opt">
                    <input name="nav_text123" id="dialog_item_image_title" type="text"
                           class="txt w200 marginright vatop">
                    <p id="dialog_image_desc" class="notic">此处为首页导航专用，其他模板无须填写</p>
                </dd>
            </dl>
            <dl class="row">
...
<script id="item_image_template" type="text/html">
    <div nctype="item_image" class="item">
        ...
        <input nctype="image_title" name="item_data[item][<%=image_name%>][title]" type="hidden"
               value="<%=image_title%>">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var $dialog_item_image1 = $('#dialog_item_image1');
         ...
         *标题图片删除
         */
        $('#item_edit_content').on('click', '[nctype="btn_del_item_image1"]', function () {
            if (!confirm('确认删除？')) {
                return false;
            }
            $.ajax({
                type: 'POST',
                url: "<?php echo BASE_SITE_URL; ?>/admin/modules/mobile/index.php?act=mb_special&op=special_item_ajax&item_id=<?php echo $output['item_info']['item_id'] ?>",
                data: "",
                dataType: "json",
                timeout: 4000,
        ...
        $("#btn_upload_image").fileupload({
            ...
            add: function (e, data) {
                ...
                if (typeof result.error === 'undefined') {
                    ...
                    $("#titleImg").attr('src', result.image_url);
                    $("#textfield").val(result.image_name)
                } else {
        ...
        $('#dialog_item_image_title').on('change', function () {
            $current_image_title.val($(this).val());
        });
