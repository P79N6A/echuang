...
class mb_specialControl extends SystemControl{
     ...
     * @return string
     */
    public function special_item_ajaxOp() {
        $model_mb_special = Model('mb_special');
        ...
        if($item_id!=false){
            $item_info = $model_mb_special->getMbSpecialItemDataByID($item_id);
            $item_info['item_data'] = unserialize($item_info['item_data']);
//            var_dump($item_info);
//            var_dump($item_info['item_data']);
            if (!empty($item_info['item_data']['textfield'])){
               unset($item_info['item_data']['textfield']);
//                var_dump($item_info);
            }else{
                echo 100;die();
            }
            $return=$model_mb_special->delMbSpecialItemTitleByID($item_info,$item_id);
            if($return){
                echo 200;die();
            }else{
                echo 0;die();
    ...
    public function special_item_editOp() {
        ...
        if (!empty($item_info['item_data']['textfield'])){
            $item_info['item_data']['img_name'] = $item_info['item_data']['textfield'];
            $item_info['item_data']['textfield'] = BASE_SITE_URL.DS."data/upload/mobile/special/s".$item_info['special_id']."/".$item_info['item_data']['textfield'];
        }
    ...
    public function special_item_saveOp() {
        $model_mb_special = Model('mb_special');
        $result = $model_mb_special->editMbSpecialItemByID(array('item_data' => $_POST['item_data']), $_POST['item_id'], $_POST['special_id'],$_POST['textfield']);
        if($result) {
