        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (!data.error) {
                    ...
                    // 渲染模板
                    var html = template.render('product_detail_add_cart', data);
                    $("#product_detail_spec_add_cart_html").html(html);
                    ...
                    $("#add-cart").click(function() {
                        ...
                        if (!key) {
                            ...
                            $('#product_detail_spec_add_cart_html').removeClass('up').addClass('dowm');
                                ...
                                success: function(result) {
                                    ...
                                    if (checkLogin(rData.login)) {
                                        if (!rData.datas.error) {
                                            ...
                                            // location.href = WapSiteUrl + '/tmpl/cart_list.html';
                                        } else {
                                            $.sDialog({
                                                skin: "red",
                    ...
                    if (data.goods_info.is_virtual == '1') {
                        $("#buy-now").click(function() {
                            ...
                            if (!key) {
                                //添加登录后，返回商品页
                                addCookie('redirect_uri', '/tmpl/product_detail.html?goods_id=' + goods_id);
                ...
                // 从下到上动态显示隐藏内容
                $.animationUp({
                    valve: '.buy-now,#goods_spec_selected', // 动作触发
                ...
                // 从下到上动态显示隐藏内容
                $.animationUp({
                    valve: '.add-cart,#goods_spec_selected', // 动作触发
                    wrapper: '#product_detail_spec_add_cart_html', // 动作块
                    scroll: '#product_roll', // 滚动块，为空不触发滚动
                    start: function() { // 开始动作触发事件
                        $('.goods-detail-foot').addClass('hide').removeClass('block');
                    },
