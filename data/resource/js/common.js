    ...
    $.getJSON(SITEURL + '/index.php?act=cart&op=del&cart_id=' + cart_id + '&callback=?', function(result) {
        if (result.state) {
            ...
            if (result.quantity == 0) {
                ...
                obj.find('.incart-goods-box').perfectScrollbar('destroy');
                obj.find('.incart-goods-box').perfectScrollbar({ suppressScrollX: true });
                $('div[ncType="rtoolbar_total_price"]').html("共计金额：<em class='goods-price'>¥" + result.amount + "</em>");
...
(function($) {
    $.fn.nc_show_dialog = function(options) {
        ...
        $(this).attr("style", "display:block; z-index: 1100; position: fixed; width: " + settings.width + "px; left: " + settings.left + "px; top: " + settings.top + "px;");
        $(this).show();
