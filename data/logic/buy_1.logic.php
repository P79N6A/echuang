...
class buy_1Logic
{
    ...
    /**
     * v跟据商品金额返回免运费店铺ID及免运费下限金额描述[已设置并达到免运费金额标准]
    ...
    public function getStoreFreightDescList($store_goods_total)
    {
        ...
        foreach ($store_list as $store_info) {
            ...
            if ($store_goods_total[$store_info['store_id']] >= $limit_price && $limit_price > 0) {
                //返回免运费金额下限
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list)
    {
        ...
        $freight_list = array();
        //v5.5 33hao
        $tpl_trans_list = array();
        foreach ($buy_list as $goods_info) {
            $transport_id = $goods_info['transport_id'];
            $goods_trans_v = $goods_info['goods_trans_v'];
            $goods_num = $goods_info['goods_num'];
            $store_id = $goods_info['store_id'];
            $_array = array();
            $_array['v'] = $goods_trans_v;
            $_array['n'] = $goods_num;
            $tpl_trans_list[$transport_id][] = $_array;
            if (empty($freight_list[$store_id])) {
                $freight_list[$store_id] = array();
            }

            if (!in_array($transport_id, $freight_list[$store_id])) {
                $freight_list[$store_id][] = $transport_id;
            }

        }
        $return['tpl_ids'] = $freight_list;
        $return['tpl_trans_list'] = $tpl_trans_list;
    ...
    public function calcStoreFreight($freight_list, $city_id)
    {
        ...
        $tpl_trans_list = $freight_list['tpl_trans_list']; //
        ...
        if (!empty($tpl_ids_list) && is_array($tpl_ids_list)) {
            ...
            foreach ($tpl_ids_list as $store_id => $transport_id_list) {
                foreach ($transport_id_list as $transport_id) {
                    $freight_total = $model_transport->calc_transport($transport_id, $city_id, $tpl_trans_list[$transport_id]);
     ...
     */
    public function beanPay($order_list, $input, $buyer_info)
    {
        $member_id = $buyer_info['member_id'];
        $member_name = $buyer_info['member_name'];
        $model_order = Model('order');
        $model_member_extend = Model('member_extend');
        $available_bean = $model_member_extend->getAvailBeanById($member_id);
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_bean >= $order_amount) {
                // 立即支付，订单支付完成
                $result = $model_member_extend->order_pay($data_pay);
                if (isset($result['error'])) {
                    throw new Exception('支付失败');
                }
                $available_bean -= $order_amount;
                $available_bean = floatval(ncPriceFormat($available_bean));
                //记录订单日志(已付款)
                $data = array();
                ...
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'beanpay';
     ...
     * @return [type]             [description]
     */
    public function sliver_beanPay($order_list, $input, $buyer_info)
    {
        $member_id = $buyer_info['member_id'];
        $member_name = $buyer_info['member_name'];
        $model_order = Model('order');
        $model_member_extend = Model('member_extend');
        $available_bean = $model_member_extend->getAvailSilverBeanById($member_id);
        $out_trade_no = $order_list[0]['pay_sn'];
        foreach ($order_list as $key => $order_info) {
            $order_amount = floatval($order_info['order_amount']);
            $data_pay = array();
            $data_pay['member_id'] = $member_id;
            ...
            if ($available_bean >= $order_amount) {
                // 立即支付，订单支付完成
                $result = $model_member_extend->sliver_order_pay($data_pay);
                if (isset($result['error'])) {
                    throw new Exception('支付失败');
                }
                $available_bean -= $order_amount;
                ...
                $insert = $model_order->addOrderLog($data);
                if (!$insert) {
                    throw new Exception('记录订单银豆支付日志出现错误');
                }
                ...
                $order_list[$key]['order_state'] = $data_order['order_state'] = ORDER_STATE_PAY;
                $order_list[$key]['payment_time'] = $data_order['payment_time'] = TIMESTAMP;
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'sliver_beanpay';
                $order_list[$key]['bean_amount'] = $data_order['bean_amount'] = $order_amount;
                ...
                // 发放金豆
                $result = Logic('payment')->orderPresentBean($out_trade_no);
                if (isset($result['error'])) {
    ...
    private function _getGoodsOnlineInfo($goods_id, $quantity)
    {
        ...
        $new_array['golden_bean']=ncPriceFormat($goods_info['goods_price']-$goods_info['goods_cost']);
        $new_array['goods_commonid'] = $goods_info['goods_commonid'];
        $goods_commoninfo=Model('goods')->getGoodsCommonInfoByID($goods_info['goods_commonid']);
        $new_array['gc_id'] = $goods_info['gc_id'];
        $new_array['store_id'] = $goods_info['store_id'];
        $new_array['goods_name'] = $goods_commoninfo['goods_name'];
        ...
        $new_array['goods_trans_v'] = $goods_info['goods_trans_v'];
    ...
    private function _getOnlineCartList($cart_list)
    {
        ...
        foreach ((array) $cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'], array_keys($goods_online_array))) {
                ...
                $goods_commoninfo=Model('goods')->getGoodsCommonInfoByID($goods_online_info['goods_commonid']);
                $cart_list[$key]['goods_name'] = $goods_commoninfo['goods_name'];
                ...
                $cart_list[$key]['golden_bean']=ncPriceFormat($goods_online_info['goods_price']-$goods_online_info['goods_cost']);
                $cart_list[$key]['transport_id'] = $goods_online_info['transport_id'];
                $cart_list[$key]['goods_freight'] = $goods_online_info['goods_freight'];
                $cart_list[$key]['goods_trans_v'] = $goods_online_info['goods_trans_v']; //
                ...
                if (is_array($_tmp_name) && is_array($_tmp_value)) {
                    ...
                    $cart_list[$key]['goods_spec_str'] = rtrim($cart_list[$key]['goods_spec'], '，');
                }
    ...
    public function balancePay($order_list, $input, $buyer_info)
    {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($balance >= $order_amount) {
                ...
                $order_list[$key]['order_state'] = $data_order['order_state'] = ORDER_STATE_PAY;
                $order_list[$key]['payment_time'] = $data_order['payment_time'] = TIMESTAMP;
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'balancepay';
                $order_list[$key]['bean_amount'] = $data_order['bean_amount'] = $order_amount;
