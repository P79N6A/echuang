...
class buyLogic
{
     ...
     * @return array
     */
    public function buyStep_bak2($post, $member_id, $member_name, $member_email, $orderdiscount = 0, $member_level = 0)
    {
...
//		if (!$post['chain']['id']) {
    ...
    private function _createOrderStep2()
    {
        ...
        if ($post['ifcart']) {
            ...
            $cart_list = $model_cart->listCart('db', $condition);
            if (empty($cart_list)){
                $condition = array('goods_id' => array('in', array_keys($input_buy_items)), 'buyer_id' => $this->_member_info['member_id']);
                $cart_list = $model_cart->listCart('db', $condition);
            }
            foreach ($cart_list as $k=>$v){
                $goods_info = Model('goods')->getGoodsInfoAndPromotionById($v['goods_id']);
                $cart_list[$k]['is_bd_product'] = $goods_info['is_bd_product'];
            }
            // 加价购条件
            $jjgObj = null;
            ...
            //商品信息[得到最新商品属性及促销信息]
            $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id, intval($quantity));
            $res = Model('goods')->getGoodsInfoAndPromotionById($goods_id);
            $goods_info['is_bd_product'] = $res['is_bd_product'];
            if (empty($goods_info)) {
    ...
    private function _createOrderStep3()
    {
        ...
        foreach ($goods_list as $key => $value) {
            $goods_freight_arr[] = $value['goods_freight'];
        }
        ...
        $this->_order_data['goods_freight'] = $goods_freight;
    }
    ...
    private function _createOrderStep4()
    {
        ...
        $member_detail = Model('member_extend')->getMemberExtendList(array('me_member_id'=>$member_id),'member_level');
        $member_level = $member_detail[0]['member_level'];
        $model_order = Model('order');
        //member_level获取折扣率
        $discount = Model('level')->getMemberDiscount($member_level);
        //存储生成的订单数据
        $order_list = array();
        //存储通知信息
        $notice_list = array();
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            foreach ($goods_list as $goods_key => $goods_value) {
                $order_amount += $goods_value['goods_num'] * $goods_value['goods_price'];
            }
            if (!empty($this->_order_data['input_voucher_list'])){
                $order['order_amount'] =$discount * ($order_amount + $goods_freight) - array_sum(array_column($this->_order_data['input_voucher_list'],'voucher_price'));
            }else{
                $order['order_amount'] =$discount * ($order_amount + $goods_freight);
            }
            $order['shipping_fee'] = $goods_freight;
            $order['goods_amount'] = $order_amount - $order['shipping_fee'] + $store_rpt_total[$store_id];
    ...
    private function _getGoodsList($cart_list)
    {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['is_bd_product'] = $cart['is_bd_product'];
