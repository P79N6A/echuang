...
//		if (!empty($sms_log) && ($sms_log['add_time'] > TIMESTAMP - DEFAULT_CONNECT_SMS_TIME)) {
//			//同一IP[n]秒内只能发一条短信
//			$state = false;
...
//		}
		...
		if ($state && !empty($sms_log) && ($sms_log['add_time'] > TIMESTAMP - DEFAULT_CONNECT_SMS_TIME)) {
//同一手机号IP[n]秒内只能发一条短信
			$state = false;
...
//		if ($state && $num >= DEFAULT_CONNECT_SMS_PHONE) {
//			//同一手机号24小时内只能发5条短信
//			$state = false;
...
//		}
		...
		$condition['add_time'] = array('egt', $time24);
		$num = $model_sms_log->getSmsCount($condition);
		if ($state && $num >= DEFAULT_CONNECT_SMS_IP) {
//同一IP24小时内只能发20条短信
			$state = false;
...
//检查手机号是否已被注册
			...
			$num = substr($phone, -4);
			$member_name = $this->getMemberName('hao', $num);
			// 分销 会员邀请
...
//查询为空表示当前会员名可用
	...
	public function wxRegister($user_info, $client) {
		...
		if (!preg_match('/^[A-Za-z0-9\x{4e00}-\x{9fa5}_]/u', $nickname)) {
			$nickname = 'wx';
		}
