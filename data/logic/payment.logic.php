...
class paymentLogic {
	...
	public function getRealOrderInfo($pay_sn, $member_id = null) {
		...
		if (empty($order_pay_info)) {
			return callback(false, '该支付订单不存在');
		}

		$order_pay_info['subject'] = '订单号：' . $order_pay_info['pay_sn'];
	...
	public function getPaymentInfo($payment_code) {
		...
		if (in_array($payment_code, array('offline', 'predeposit')) || empty($payment_code)) {
			return callback(false, '系统不支持选定的支付方式1');
		}
        $model_payment = Model('payment');//payment_list表
        $condition = array();
        $condition['payment_code'] = $payment_code;
        $payment_info = $model_payment->getPaymentOpenInfo($condition);
        if (empty($payment_info)) {
            return callback(false, '系统不支持选定的支付方式2');
        }
        //强制修改支付路径（微信、支付宝）
        if($payment_info['payment_code'] == "alipay_scan"){
            $payment_info['payment_code'] = "alipay";
        }else if($payment_info['payment_code'] == "wxpay_scan"){
            $payment_info['payment_code'] = "wxpay";
        }

//		echo $payment_info['payment_code'];exit;
//		$inc_file = BASE_PATH . DS . 'api' . DS . 'payment' . DS . $payment_info['payment_code'] . DS . $payment_info['payment_code'] . '.php';
        $inc_file = BASE_PATH . DS . 'api' . DS . 'payment' . DS .$payment_info['payment_code'] . DS . $payment_info['payment_code'] . '.php';
        //echo $inc_file;
        if (!file_exists($inc_file)) {
            return callback(false, '系统不支持选定的支付方式3');
        }
	 ...
	 * @param unknown $payment_code
	 */
	public function getPaymentReturnInfo($payment_code) {
	    echo $payment_code;
		if (in_array($payment_code, array('offline', 'predeposit')) || empty($payment_code)) {
        ...
        $condition = array();
        $condition['payment_code'] = $payment_code;
        $payment_info = $model_payment->getPaymentReturnInfo($condition);
        if (empty($payment_info)) {
            return callback(false, '系统不支持选定的支付方式');
        }
        //强制修改支付路径（微信、支付宝）
...
//		$inc_file = BASE_PATH . DS . 'api' . DS . 'payment' . DS . $payment_info['payment_code'] . DS . $payment_info['payment_code'] . '.php';
	 ...
	 */
	public function orderPresentBean($out_trade_no) {
		$model_order = Model('order');
		$condition = array();
		...
		foreach ($order_list as $order_info) {
			Logic('order')->orderPresentBean($order_info);
		}
	}
