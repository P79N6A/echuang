...
//			}
	...
	public function addInviteRate($order_info) {
		...
		if ($commis_amount_sum > 0) {
			...
			$im_arr = explode('.', $commis_amount_sum * $GLOBALS['setting_config']['hao_invite2'] * 0.01);
			$invite_money2 = $im_arr[0] . '.' . substr($im_arr[1], 0, 2);
			$im_arr = explode('.', $commis_amount_sum * $GLOBALS['setting_config']['hao_invite3'] * 0.01);
		...
		if ($model_payment->is_online_pay($order_info['payment_code'])) {
			...
			foreach ($order_goods_list as $order_goods_info) {
				...
				if (!empty($goods_info)) {
					...
					$goods_costprice = $goods_info['goods_cost'] >0? $goods_info['goods_cost'] : 0;
					$goods_num = $order_goods_info['goods_num'];
					$present_bean_amount += floatval(($goods_price * $goods_num * 100 - ($goods_costprice * $goods_num) * 100) / 100);
				}
			}

			if ($present_bean_amount > 0) {
				$model_member_extend = Model('member_extend');
				$bean_data = array();
				$bean_data['member_id'] = $member_id;
				$info = $model_member_extend->getMemberInfo(array('member_id' => $member_id), 'member_name');
				$bean_data['member_name'] = $info['member_name'];
				$bean_data['amount'] = $present_bean_amount;
				$result = $model_member_extend->changeBean('present', $bean_data);
			}
		}
	}
}