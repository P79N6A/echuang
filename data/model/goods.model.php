...
class goodsModel extends Model {
	...
	public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0) {
		...
		if (C('dbdriver') == 'mysqli') {
			...
			$_field = "CONCAT(goods_commonid) as nc_distinct ," . $field;
...
//        var_dump($res);die();
	...
	/**
	 * getGoodsAndCostprice 获取商品成本价
	 * @param  [type] $goods_commonid [description]
	 * @return [type]                 [description]
	 */
	public function getGoodsAndCostprice($goods_commonid) {
		return $this->getGoodsCommonInfoByID($goods_commonid, 'goods_price,goods_costprice');
		// return $info['goods_costprice'] ? $info['goods_costprice'] : 0;
	}
     ...
     * @return int
     */
    public function getGoodsCommonProductCount($condition) {
        $condition['is_bd_product'] = 1;
        return $this->getGoodsCommonCount($condition);
    }
	...
	public function getGoodsDetail($goods_id) {
		...
		if ($result1['goods_body'] == '') {
			unset($result1['goods_body']);
		}

		if ($result1['mobile_body'] == '') {
		...
		$goods_info['goods_name'] = $result2['goods_name'];
		$goods_info['spec_value'] = unserialize($goods_info['spec_value']);
		$goods_info['spec_name'] = unserialize($goods_info['spec_name']);
		if (is_array($goods_info['spec_name']) && $goods_info['spec_name']) {
			foreach ($goods_info['spec_name'] as $name_key => $name_value) {
				$spec_name[] = $name_value;
			}
		}

		$goods_info['goods_spec'] = unserialize($goods_info['goods_spec']);
		if (is_array($goods_info['goods_spec']) && $goods_info['goods_spec']) {
			foreach ($goods_info['goods_spec'] as $goods_spec_key => $goods_spec_value) {
				$goods_spec[] = $goods_spec_value;
			}
		}

		$goods_spec_str = "";
		for ($i = 0; $i < count($spec_name); $i++) {
			$goods_spec_str .= $spec_name[$i] . ":" . $goods_spec[$i] . "   ";
		}
		$goods_info['goods_spec_str'] = $goods_spec_str;
		...
		if (is_array($goods_info['spec_name']) && $goods_info['spec_name']) {
			foreach ($goods_info['spec_name'] as $key => $value) {
				$spec_info['spec_title'] = $goods_info['spec_name'][$key];
				$spec_info['spec_content_list'] = $goods_info['spec_value'][$key];
				$spec_list[] = $spec_info;
			}
		}

		$goods_info['spec_style_list'] = $spec_list;
		if (is_array($goods_info['spec_style_list']) && $goods_info['spec_style_list']) {
			foreach ($goods_info['spec_style_list'] as $k => $v) {
				foreach ($v['spec_content_list'] as $spec_key => $spec_value) {
					$spec_info_arr['spec_id'] = $spec_key;
					$spec_info_arr['spec_name'] = $spec_value;
					$goods_info['spec_style_list'][$k]['spec_content'][] = $spec_info_arr;
				}
			}
		}
		...
		//金豆排序
		$goods_info['golden_bean'] = ncPriceFormat($goods_info['goods_price']-$goods_info['goods_cost']);
		...
		$goods_info['golden_min_bean']=!empty($gold_bean['golden_min_bean'])? $gold_bean['golden_min_bean']:"0";
		$goods_info['golden_max_bean']=!empty($gold_bean['golden_max_bean'])? $gold_bean['golden_max_bean']:"0";
	 ...
	 * 处理商品联合查询
	 */
	public function getGoodsInfoJoinGoodsCommon($condition) {
		return $this->table('goods_common,goods')
			->field('*')
			->where($condition)
			->join('inner')
     ...
     * @return array
     */
	public function getProductNum($pay_sn){
        $goods_info_arr = $this
            ->table('orders,order_goods')
            ->field('orders.buyer_id,order_goods.goods_id,order_goods.goods_num,orders.order_sn')
            ->where(array('orders.pay_sn'=>$pay_sn))
            ->join('union')
        ...
        if (!empty($goods_info_arr)){
            $order_sn = $goods_info_arr[0]['order_sn'];
            $member_id = $goods_info_arr[0]['buyer_id'];
            foreach ($goods_info_arr as $key=>$value){
                $is_product_goods = $this->table('goods')->where(array('goods_id'=>$value['goods_id']))->field('is_bd_product')->find();
                $product_num = $product_num + $is_product_goods['is_bd_product'] * $value['goods_num'];
            }
        }
     ...
     * @param $bd_product
     */
    public function changeProductState($commonid,$bd_product){
	    $res = $this->table('goods')->where(array('goods_commonid'=>$commonid))->update(array('is_bd_product'=>$bd_product));
	    $this->table('goods_common')->where(array('goods_commonid'=>$commonid))->update(array('is_bd_product'=>(int)$bd_product));
	    return $res;
    }

    public function getGoodsIdArr($condition){
        return $this->table('goods')->field('goods_id')->where($condition)->select();
    }
}
