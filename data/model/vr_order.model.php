...
class vr_orderModel extends Model {
    ...
    public function addOrderCode($order_info) {
        ...
        $dir = BASE_UPLOAD_PATH.DS.ATTACH_PATH.DS.'vr_code'.DS.date('Y-m-d').DS;
        if(!is_dir($dir)){
            mkdir($dir,0755,true);
        }
        for ($i=0; $i < $order_info['goods_num']; $i++) {
            ...
            $file_name = $order_info['store_id'].'-'.$order_info['buyer_id'].'-'.md5($code_list[$i].rand(100,999)).'.png';
            $order_code[$i]['qr_pic'] = date('Y-m-d').DS.$file_name;
            
            $PhpQRCode = new PhpQRCode();
            $PhpQRCode->set('pngTempDir',$dir);
            $PhpQRCode->set('date', $code_list[$i]);
...
//         $order_code[$i-1]['pay_price'] += $diff_amount;
    ...
    public function getOrderCodeList($condition = array(), $fields = '*') {
        ...
        if (!empty($code_list)) {
			...
			//v5.5 二维码
                        if (!empty($v['qr_pic'])) {
                            $code_list[$k]['qr_pic_url'] = UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/vr_code/'.$v['qr_pic'];
                        }
                    }
                }
                if ($v['refund_lock'] == '1') {
