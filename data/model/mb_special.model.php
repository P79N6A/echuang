...
class mb_specialModel extends Model
{
    ...
    */
     ...
     * 首页专题
     */
    public function getMbSpecialIndex($special_id = '')
    {
        if (!empty($special_id)) {
            return $this->getMbSpecialItemUsableListByID($special_id);
        } else {
            $res = $this->getMbSpecialItemUsableListByID(self::INDEX_SPECIAL_ID);
            return $res;
        }
    }
    ...
    private function _formatMbSpecialData($item_data, $item_type)
    {
        switch ($item_type) {
            case 'nav1':
                foreach ($item_data['square_image'] as $k => $v) {
                    if ($v != '') {
                        $item_data['square_image'][$k] = getMbSpecialImageUrl($v);
                    }
                }
                break;
            case 'home1':
...
//            var_dump($item_list[$key]['item_data']);
...
//        die();
    ...
    private function _initMbSpecialItemData($item_data, $item_type)
    {
        if (!empty($item_data)) {
            ...
            if ($item_type == 'goods' || $item_type == 'goods2') {
                $item_data = $this->_initMbSpecialItemGoodsData($item_data, $item_type);
            }
            if ($item_type == 'goods1') {
                $item_data = $this->_initMbSpecialItemGoods1Data($item_data, $item_type);
            }
        } else {
     ...
     * 处理限时goods类型内容
     */
    private function _initMbSpecialItemGoods1Data($item_data, $item_type)
    {
        $goods_id_string = '';
        if (!empty($item_data['item'])) {
            foreach ($item_data['item'] as $value) {
            ...
            $goods_list = $model_goods->getXianshiGoodsList($condition, 'goods_id,goods_name,xianshi_price,goods_image');
            $goods_list = array_under_reset($goods_list, 'goods_id');
            //整理商品数据
            $new_goods_list = array();
            foreach ($item_data['item'] as $value) {
                $goods_list[$value]['time'] = $goods_list[$value]['end_time'] - TIMESTAMP;
                if (!empty($goods_list[$value])) {
                    $new_goods_list[] = $goods_list[$value];
    ...
    */
     ...
     *
     */
    public function editMbSpecialItemByID($update, $item_id, $special_id, $textfield = null)
    {
        if (isset($update['item_data'])) {
            ...
            $update['item_data'] = serialize($data);
        }
        $condition = array();
        $condition['item_id'] = $item_id;
    ...
    public function delMbSpecialItemTitleByID($item_info, $item_id){
//        var_dump($item_info);
        $update1['item_data'] = serialize($item_info['item_data']);
//        var_dump($update1['item_data']);die();
        $condition['item_id'] = $item_id;
        return $this->table('mb_special_item')->where($condition)->update($update1);
    }

    public function getMbSpecialItemDataByID($item_id){
        return $this->table('mb_special_item')->where(array('item_id'=>$item_id))->find();
    }
    ...
    */
    ...
    {
        $module_list = array();
        $module_list['adv_list'] = array('name' => 'adv_list', 'desc' => '轮播图版块');
        $module_list['nav1'] = array('name' => 'nav1', 'desc' => '首页导航列表1');
        $module_list['nav2'] = array('name' => 'nav2', 'desc' => '首页导航列表2');
