...
//        var_dump($this->table('templates_left')->where($condition)->field($fields)->find());die();
        return $this->table('templates_left.model')->where($condition)->field($fields)->find();
    }
    
public function get_goods_gallery_album($type = 0, $id = 0, $select = array(), $id_name = 'album_id', $order = '')
{
//    var_dump($this->db->getOne());die();
	$where = 1;
    $tablePre = C('tablepre');
	if ($id) {
	...
	if ($type == 1) {
		...
		$album_list = $this->db->getOne($sql);
	}
//var_dump($album_list);die();
	return $album_list;
}
