...
class storeModel extends Model {
     ...
     * @return array
     */
    public function getStoreMemberIDList($storeid_array, $field = 'store_id,member_id,store_domain,store_qq') {
...
/**
     ...
     */
    public function getStoreSearchList($store_array,$numlimit =8) {
    	...
    	if(!empty($store_array)){
    		...
    		if(!empty($no_cache_store)) {
    			...
    			//获取店铺推荐商品
    			$no_cache_store = $this->getGoodsListBySales($no_cache_store,$numlimit);
    			//写入缓存
    ...
    //获取店铺8个销量最高商品
    private function getGoodsListBySales($store_array,$numlimit) {
    	$model = Model();
    	$field = 'goods_id,store_id,goods_name,goods_image,goods_price,goods_salenum';
    	foreach ($store_array as $value) {
    		$store_array[$value['store_id']]['search_list_goods'] = $model->table('goods')->field($field)->where(array('store_id'=>$value['store_id'],'goods_state'=>1))->order('goods_salenum desc')->limit($numlimit)->select();
    	}
    	return $store_array;
    }
}
