...
//        $member_info = $model_member->getMemberInfoByID($member_id);
        ...
        $member_info = $model_member_extend->getMemberExtendInfo(array('member_id'=>$member_id),'*', 'union');
...
//        }
        if ($member_info['member_level'] < intval($template_info['voucher_t_mgradelimit'])){
            $member_level_name = str_replace(array(0,1,2,3,4,5,),array("体验用户","VIP","店主","合伙人","高级合伙人","战略合伙人"),$template_info['voucher_t_mgradelimit']);
            return array('state'=>false,'msg'=>'该代金券限'.$member_level_name.'及'.$member_level_name.'以上可兑换');
        }
        //查询代金券对应的店铺信息
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null, $order = 'voucher_id desc') {
        ...
        $field = 'voucher_id,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_order_id,voucher_store_id,store_name,store_id,store_domain,store_label,store_avatar,is_own_shop,voucher_t_customimg';
...
//        }
     ...
     * 条件兑换代金券
     */
    public function exchangeVoucherByCondition($template_info, $member_id, $member_name = ''){
        if (intval($member_id) <= 0 || empty($template_info)){
            return array('state'=>false,'msg'=>'参数错误');
        }
        //查询会员信息
