...
class orderModel extends Model {
	...
	//处理微信支付完成，改变order_pay表状态
	public function updateWxPayStart($out_trade_no){
		$data = array(
			"pay_sn" =>$out_trade_no,
		);
		$start = array(
		...
		$this->table('order_pay')->where($data)->update($start);
	}
	...
	public function editOrderCountCache($type, $id, $data) {
		if (!C('cache_open') || empty($type) || !intval($id) || !is_array($data)) {
			return;
		}
	...
	public function addOrderCommon($data) {
	    try{
            $res = $this->table('order_common')->insert($data);
        }catch (Exception $e){
	       return false;
        }
        return $res;
	...
	/**
	 * getWxPayAmount 获取微信支付金额
	 * @param  [type] $condition [description]
	 * @return [type]            [description]
	 */
	public function getWxPayAmount($condition) {
		$condition['order_state'] = array('in', array(20, 30, 40));
		$condition['payment_code'] = array('in', array('wxpay', 'wx_saoma'));
		return $this->table('orders')->where($condition)->sum('order_amount');
	}

	/**
	 * getWxPayCountNum 获取微信支付条数
	 * @param  [type] $condition [description]
	 * @return [type]            [description]
	 */
	public function getWxPayCountNum($condition) {
		$condition['order_state'] = array('in', array(20, 30, 40));
		$condition['payment_code'] = array('in', array('wxpay', 'wx_saoma'));
		return $this->table('orders')->where($condition)->count();
	}
	...
	/**
	 * getAliPayAmount 获取支付宝支付金额
	 * @param  [type] $condition [description]
	 * @return [type]            [description]
	 */
	public function getAliPayAmount($condition) {
		$condition['order_state'] = array('in', array(20, 30, 40));
		$condition['payment_code'] = array('in', array('alipay_native_app', 'alipay'));
		return $this->table('orders')->where($condition)->sum('order_amount');
	}

	/**
	 * getAliPayCountNum 获取支付宝支付条数
	 * @param  [type] $condition [description]
	 * @return [type]            [description]
	 */
	public function getAliPayCountNum($condition) {
		$condition['order_state'] = array('in', array(20, 30, 40));
		$condition['payment_code'] = array('in', array('alipay_native_app', 'alipay'));
		return $this->table('orders')->where($condition)->count();
	}
	...
	*获取报单产品订单号
	*/
	public function getProductOrder($condition){
	    return $this->table('order_goods,goods')->where($condition)->field('order_goods.order_id')->join('union')->on('order_goods.goods_id=goods.goods_id')->select();
    }
	//支付宝支付完成改变订单状态
	public function updateAlipayOrdersStart($wheres){
		$data = array(
			"order_state"=>'20',
		);
		return $this->table('orders')->where($wheres)->update($data);
	}
     ...
     *获取订单和商品订单详细信息
     */
    public function getOrderEndOrdergoodsInfo($condition){
        return $this->table('order_goods,orders')->where($condition)->join('union')->on('order_goods.order_id=orders.order_id')->select();
    }
