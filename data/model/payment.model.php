...
class paymentModel extends Model {
	...
	public function getPaymentOpenInfo($condition = array()) {
		...
		return $this->table('payment_list')->where($condition)->find();
	}
	 ...
	 * @return array 数组格式的返回结果
	 */
	public function getPaymentReturnInfo($condition = array()) {
		$condition['payment_state'] = self::STATE_OPEN;
		return $this->table('mb_payment')->where($condition)->find();
	}
	...
	public function getPaymentOpenList($condition = array()) {
		$condition['payment_state'] = self::STATE_OPEN;
		$res = $this->where($condition)->key('payment_code')->select();
//	    var_dump($res);die();
		return $res;
	}
