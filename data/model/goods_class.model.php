...
class goods_classModel extends Model {
	...
	public function get_child_category($id = '') {
		...
		$result = $this->table('goods_class')->field('*')->where($condition)->order('gc_id desc')->limit(false)->select();
		return $result;
	}
	/**
	 * 类别列表
...
//$i为上次循环到的分类编号，避免重新从第一条开始
				...
				if ($gc_parent_id == $parent_id) {
					$val['deep'] = $deep;
					$show_class[] = $val;
					if ($deep < $show_deep && $deep < 3) {
//本次深度小于显示深度时执行，避免取出的数据无用
...
//当前分类的父编号大于本次递归的时退出循环
	 ...
	 * @return array $nav_link 返回数组形式类别导航连接
	 */
	public function getGoodsClassNavsearch($id = 0, $sign = 1) {
		if (intval($id) > 0) {
			$data = $this->getGoodsClassIndexedListAll();
