...
class memberModel extends Model {
    ...
    /**
     * 获取某个或各个等级会员的总人数
     * @param $level integer(0-6) 会员等级参数
     * @return array
     */
	 ...
	 */
	public function createSession($member_info = array(), $reg = false, $update = true) {
	...
	public function login($login_info) {
		...
		if (empty($member_info) && preg_match('/^0?(13|15|16|17|18|14)[0-9]{9}$/i', $user_name)) {
			//根据会员名没找到时查手机号
			$condition = array();
		...
		// if (empty($member_info) && (strpos($user_name, '@') > 0)) {
		//     // 按邮箱和密码查询会员
		//     $condition = array();
	 ...
	 * 注册
	 */
	public function register($register_info, $type = '') {
		// 重复注册验证
		...
		if (empty($type)) {
			...
			$member_info['invite_three'] = $register_info['invite_three'];
			// 会员系统
			$member_info['invite_id'] = $register_info['invite_id'];
			$member_info['access_id'] = $register_info['access_id'];
			$member_info['depth'] = $register_info['depth'];
			$member_info['member_level'] = $register_info['member_level'];
			$insert_id = $this->addMember($member_info, $type);
			if ($insert_id) {
				$member_info['member_id'] = $insert_id;
				$member_info['is_buy'] = 1;
				process::addprocess('reg');
	 ...
	 * @return  array 数组格式的返回结果
	 */
	public function addMember($param, $type = '') {
		if (empty($param)) {
			return false;
		}
		try {
			...
			// 会员系统
			if ($type == 'member_system') {
				$member_system_info = array();
				$member_system_info['me_member_id'] = $insert_id;
				$member_system_info['invite_id'] = $param['invite_id'];
				$member_system_info['access_id'] = $param['access_id'];
				$member_system_info['depth'] = $param['depth'];
				$member_system_info['reg_time'] = TIMESTAMP;
				$member_system_info['member_level'] = $param['member_level'];

				$add_id = Model('member_extend')->addMemberExtend($member_system_info);
				dump($add_id);
				die;
			}
			$this->commit();
	...
	/**
	 * getMemberMobileById 获取手机号
	 * @param  [type] $member_id [description]
	 * @return [type]            [description]
	 */
	public function getMemberMobileById($member_id) {
		$info = $this->getMemberInfoByID($member_id);
		if (!empty($info)) {
			return $info['member_mobile'];
		} else {
			return '';
		}
	}
     ...
     * @return mixed
     */
	public function addInventory($condition,$money){
	    return $this->where($condition)->setInc('inventory',$money);
    }
     ...
     * @return mixed
     */
    public function getDirectParentID($condition,$fields = '*'){
     ...
     * @return mixed
     */
    public function addBalance($member_id,$money){
        return $this->table('member')->where(array('member_id'=>(int)$member_id))->setInc("balance",$money);
    }
    /**
     * 获取会员信息
     * @param $condition
     ...
     * @return mixed
     */
    public function getInfoMember($condition,$fields){
        return $this->where($condition)->field($fields)->find();
    }
     ...
     * @return mixed
     */
    public function getAvailBalanceById($member_id){
        $res = $this->where(['member_id'=>$member_id])->field('balance')->find();
        return $res['balance'];
    }
     ...
     * @return mixed
     */
    public function getMemberBonus($condition,$fields='*'){
     ...
     * @return mixed
     */
    public function getMemberInventory($condition){
        return $this->where($condition)->order('member_id desc')->field('member_id,inventory')->find();
    }
     ...
     * @param $inventory
     */
    public function changeMemberInventory($condition,$inventory){
        return $this->where($condition)->setDec('inventory',$inventory);
    }

	//添加会员分红
    public function addMemberBonus($member_id,$bonus){
        return $this->where(array('member_id'=>$member_id))->setInc('bonus',$bonus);
    }


    //获取各会员等级对应的加盟费数组
    public function getFranchiseFee($member_level){
        ...
        if ($member_level == 0){
            $return_data = $franchise_fee_arr;

        }else{
            foreach ($franchise_fee_arr as $k=>$v){
                $return_data[$k] = $franchise_fee_arr[$k]['ml_franchise_fee'] - $franchise_fee_arr[$member_level]['ml_franchise_fee'];
                if ($return_data[$k] <= 0){
                    $return_data[$k] = $franchise_fee_arr[$k]['ml_franchise_fee'];
                }
            }
        }
        return $return_data;
    ...
    public function getMemberBalanceById($member_id){
        $res =  $this->table('member')->where(array('member_id'=>$member_id))->field('balance')->find();
        return $res['balance'];
    }
     ...
     * @return mixed
     */
    public function getMemberBounsById($member_id){
        $res =  $this->table('member')->where(array('member_id'=>$member_id))->field('bonus')->find();
        return $res['bonus'];
    }
     ...
     * @return mixed
     */
    public function getMemberInventoryById($member_id){
        $res =  $this->table('member')->where(array('member_id'=>$member_id))->field('inventory')->find();
