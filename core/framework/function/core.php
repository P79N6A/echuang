...
//        var_dump($image);die();
    ...
    if ($retain) {
        ...
        $url = (empty($image) ? DS.'gallery_album'.DS.'errorImg.png' : $image);
    }
 ...
 */
function Plugin(array $args) {
	if (empty($args)) {
		$error = 'Plugin Error: arguments is empty';
		throw_exception($error);
	}
	$args = array_values($args);
	$count = count($args);
	$class_name = $args[$count - 1];
	$cache_key = $class_name;
	$base_path = BASE_DATA_PATH . '/plugin';
	$file_name = $base_path;
	for ($i = 0; $i < $count - 1; $i++) {
		$file_name .= '/' . $args[$i];
	}
	$file_name .= '/' . $class_name . '/' . $class_name . '.php';
	if (!file_exists($file_name)) {
		return $_cache[$cache_key] = new Model($class_name);
	}

	require_once $file_name;
	if (!class_exists($class_name)) {
		$error = 'Plugin Error:  Class ' . $class_name . ' is not exists!';
		throw_exception($error);
	}
	// } else {
	// 	return $_cache[$cache_key] = new $class_name();
...
function showEditor($id, $value = '', $width = '700px', $height = '300px', $style = 'visibility:hidden;', $upload_state = "true", $media_open = false, $type = 'all') {
	...
	echo '<textarea id="' . $name . '" name="' . $name . '" style="width:' . $width . ';height:' . $height . ';' . $style . '">' . $value . '</textarea>';
...
/**
 * defaultCategroyImage 取得默认分类图片
 * @param  [type] $key [description]
 * @return [type]      [description]
 */
function defaultCategroyImage($key) {
	$file = str_ireplace('.', '_' . $key . '.', C('default_categroy_image'));
	return ATTACH_COMMON . DS . $file;
}
...
function getCMSSpecialHtml($special_id) {
	$special_file = file_get_contents(UPLOAD_SITE_URL . DS . ATTACH_CMS . DS . 'special_html' . DS . md5('special' . intval($special_id)) . '.html');
	return $special_file;
}
...
function pagecmd($cmd = '', $arg = '') {
	...
	case 'setstyle':$page->setStyle($arg);
		break;
	case 'show':return $page->show($arg);
		break;
	...
	case 'gettotalpage':return $page->getTotalPage();
		break;
	case 'getnowpage':return $page->getNowPage();
		break;
	case 'settotalpagebynum':return $page->setTotalPageByNum($arg);
		break;
	default:break;
	}
}
...
function getChat($layout) {
	if (!C('node_chat') || !file_exists(BASE_CORE_PATH . '/framework/libraries/chat.php')) {
		return '';
	}
...
*/
	...
	if ($act == 'search' && $op == 'index' && $args['cate_id'] > 0 && empty($args['keyword'])) {
//商品搜索列表页只有商品分类参数
		$id = intval($args['cate_id']);
		$channel_list = rkcache('channel', true);
		if ($channel_list[$id] > 0) {
//商品分类与频道的绑定
			$act = 'channel';
			$args = array();
...
function urlAdminFlea($act = '', $op = '', $args = array()) {
	return url($act, $op, $args, false, ADMIN_SITE_URL . DS . ADMIN_MODULES_FLEA);
}
...
/**
 * 会员系统使用的URL链接函数，强制使用动态传参数模式
 * @param  string $act  [description]
 * @param  string $op   [description]
 * @param  array  $args [description]
 * @return [type]       [description]
 */
function urlMemberSystem($act = '', $op = '', $args = array()) {
	return url($act, $op, $args, false, MEMBER_SYSTEM_SITE_URL);
}
...
if (!function_exists('makeSN')) {
	...
	function makeSN($type, $member_id) {
		$sn = mt_rand(10, 99)
...
/**
 * replaceBeanType 奖金名称替换
 * @param  [type] $bean_type [description]
 * @return [type]            [description]
 */
if (!function_exists('replaceBeanType')) {
	function replaceBeanType($bean_type) {
		return str_replace(array('rebate', 'consume', 'withdraw', 'admin', 'recharge', 'donate', 'present', 'order_pay', 'dividend', 'promotion', 'extend', 'service', 'order_pay'), array('分享奖励', '消费', '提现', 'redbag', '充值', '转赠', '赠送', '订单支付', '商城赠送', '推广奖', '拓展奖', '服务中心奖', '订单支付'), $bean_type);
	}
}

/**
 * replaceEquityType 金券类型替换
 * @param  [type] $equity_type [description]
 * @return [type]              [description]
 */
if (!function_exists('replaceEquityType')) {
	function replaceEquityType($equity_type) {
		return str_replace(array('consume', 'admin', 'capping'), array('消费', 'redbag', '封顶'), $equity_type);
	}
}

/**
 * make_QR_code 生成二维码
 * @param  [type] $code_dir  [description]
 * @param  [type] $code_name [description]
 * @param  [type] $data      [description]
 * @return [type]            [description]
 */
if (!function_exists('make_QR_code')) {
	function make_QR_code($code_dir, $code_name, $data) {
		$img_file = BASE_UPLOAD_PATH . DS . $code_dir . DS . $code_name . '.php';
		if (!file_exists($img_file)) {
			require_once BASE_RESOURCE_PATH . DS . 'phpqrcode' . DS . 'index.php';
			$phpQRCode = new PhpQRCode();
			$phpQRCode->set('pngTempDir', BASE_UPLOAD_PATH . DS . $code_dir . DS);
			$phpQRCode->set('date', $data);
			$phpQRCode->set('pngTempName', $code_name . '.png');
			$phpQRCode->init();
		}
		return UPLOAD_SITE_URL . '/' . $code_dir . '/' . $code_name . '.png';
	}
}
...
if (!function_exists('formatPhoneNo')) {
	function formatPhoneNo($phoneNo) {
		...
		// 截取手机号后4位
		$suffix = substr($phoneNo, -4, 4);

		$maskPhoneNo = $prefix . " ****" . $suffix;
...
/**
 * manualDelCacheFile 手动删除缓存文件
 * @param  [type] $filename [description]
 * @param  [type] $dir      [description]
 * @return [type]           [description]
 */
if (!function_exists('manualDelCacheFile')) {
	function manualDelCacheFile($filename, $dir = null) {
		if (!is_null($filename)) {
			$file = '';
			if (is_null($dir)) {
				$file = BASE_DATA_PATH . DS . 'cache' . DS . $filename . '.php';
			} else {
				$file = BASE_DATA_PATH . DS . 'cache' . DS . $dir . DS . $filename . '.php';
			}
			if (file_exists($file)) {
				@unlink($file);
			}
...
if (!function_exists('responseJson')) {
	 ...
	 * @return [type]         [description]
	 */
	function responseJson($state = 0, $data = array(), $msg = '') {
		exit(json_encode(array('state' => $state, 'data' => $data, 'msg' => $msg)));
	}
}

if (!function_exists('checkIsLinuxOs')) {
	/**
	 * checkIsLinuxOs 查看当前系统是否为Linux系统
	 * @return [type] [description]
	 */
	function checkIsLinuxOs() {
...
/**
 * convertToEquityNum 金券转换为金券数量
 * @param  [type] $equity [description]
 * @param  [type] $min    [description]
 * @param  [type] $max    [description]
 * @return [type]         [description]
 */
if (!function_exists('convertToEquityNum')) {
	function convertToEquityNum($equity, $min, $max) {
		$num = 0;
		if ($equity >= $max) {
			$num = 10;
		} elseif ($equity >= $min) {
