...
class cls_image
{
	...
	public function upload_image($upload, $dir = '', $img_name = '', $steps = 0, $name = '', $type = '', $tmp_name = '', $error = '', $size = '')
	{
		...
		$admin_dir = date('Ym');
		$admin_dir = BASE_ROOT_PATH . DS . $this->images_dir . '/' . $admin_dir . '/' . 'admin_' . $admin_id;
		...
		if (empty($dir)) {
			$dir = date('Ym');
			$dir = BASE_ROOT_PATH . DS . $this->images_dir . '/' . $dir . '/' . 'admin_' . $admin_id . '/';
		}
		else {
			$dir = BASE_ROOT_PATH . DS . $this->data_dir . '/' . $dir . '/';
		...
		if ($this->move_file($upload, $img_name)) {
			if ($upload_type) {
				...
				return str_replace(BASE_ROOT_PATH . DS, '', $img_name);
			}
		}
		else {
	...
	public function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor = '', $filename = '')
	{
		...
		if (($thumb_width == 0) && ($thumb_height == 0)) {
			return str_replace(BASE_ROOT_PATH . DS, '', str_replace('\\', '/', realpath($img)));
		}
		...
		if (empty($path)) {
			...
			$dir = BASE_ROOT_PATH . DS . $this->images_dir . '/' . date('Ym') . '/' . 'admin_' . $admin_id . '/';
		}
		else {
			$dir = $path;
		}
		...
		if (file_exists($dir . $filename)) {
			if ($upload_type) {
				...
				return str_replace(BASE_ROOT_PATH . DS, '', $dir) . $filename;
			}
		}
		else {
	...
	public function add_watermark($filename, $target_file = '', $watermark = '', $watermark_place = '', $watermark_alpha = 0.65000000000000002)
	{
		...
		if (($watermark_place == 0) || empty($watermark)) {
			return str_replace(BASE_ROOT_PATH . DS, '', str_replace('\\', '/', realpath($filename)));
		}
		...
		if ($path) {
			return str_replace(BASE_ROOT_PATH . DS, '', str_replace('\\', '/', $path));
		}
		else {
			$this->error_msg = '图片写入失败 ';
			$this->error_no = 4;
