...
class indexControl extends apiEncryptCommonControl {
	...
	private function _output_special($data, $type = 'json', $special_id = 0) {
	    ...
	    foreach ($data as $k=>$v) {
            if (array_key_exists('home1',$v) || array_key_exists('home2',$v) || array_key_exists('home3',$v) || array_key_exists('home4',$v) || array_key_exists('goods1',$v) || array_key_exists('goods2',$v)){
	...
	public function member_tokenOp() {
		...
		$member_info = Model('member_extend')->getMemberExtendInfo(array('member_id'=>$mb_user_token_info['member_id']),'*','union');
		echo json_encode(array('member_avatar'=>$member_info['member_avatar'],'member_id' => $member_info['member_id'], 'member_mobile' => $member_info['member_mobile'], 'member_name' => $member_info['member_name'],'total_bd_product_consume'=>$member_info['total_bd_product_consume']));
	}
