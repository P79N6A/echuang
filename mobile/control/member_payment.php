...
class member_paymentControl extends mobileMemberControl {
	 ...
	 */
	public function getPaymentListOp() {
		$paylist = Model('payment_list')->getWapPaymentList();
		output_data(['list' => $paylist]);
	}
...
//			}
	...
	public function wx_pay($pay_info){
        // 第三方支付
        Plugin(['payment', 'wxpay', 'wxpay_wap']);
        $data = array(
            'body' => $pay_info['subject'],
            'subject' => $pay_info['subject'],
            'out_trade_no' => $pay_info['pay_sn'],
            'total_amount' => $pay_info['api_pay_amount'],
            'notify_url' => "http://shop.divona.xin/shop/api/payment/wxpay/notify_url.php",
        );
	...
	// 网页支付
	public function wapPayOp() {
        $pay_sn = $_GET['pay_sn'];
        if (!preg_match('/^\d{18}$/', $pay_sn)) {
            output_error('支付订单号格式错误');
        }

        $payment_code = $_GET['payment_code'];
        $payment_info = Model('payment_list')->getPaymentOpenInfo(['payment_code' => $payment_code]);
        if (empty($payment_info)) {
			output_error("系统不支持所选支付方式");
		}
		$pay_info = $this->_get_real_order_info($pay_sn);
		if (isset($pay_info['error'])) {
			output_error($pay_info['error']);
		...
		// 验证订单信息
		$order_list = $pay_info['data']['order_list'];
		if ($payment_info['payment_tag'] == 0) {
			if (!isset($_GET['pay_passwd'])) {
				output_error('请输入支付密码');
			}
			$pay_passwd = $_GET['pay_passwd'];
			$buyer_info = Model('member')->getMemberInfoByID($this->member_info['member_id']);
			if ($buyer_info['member_paypwd'] == '' || $buyer_info['member_paypwd'] != md5($pay_passwd)) {
				output_error('用户支付密码不正确');
			}
			$post = array();
			if ($payment_code == 'goldpay') {
				// 金豆支付
				try {
					$result = Logic('buy_1')->beanPay($order_list, $post, $buyer_info);
					if ($result) {
						output_data(1);
					} else {
						output_error('支付失败');
				...
				// 银豆支付
				try {
					$result = Logic('buy_1')->sliver_beanPay($order_list, $post, $buyer_info);
					if ($result) {
						output_data(1);
					} else {
						output_error('支付失败');
			...
			// 第三方支付
            $payment_class = $payment_info['payment_class'];
            if ($payment_code == 'alipay_wap'){
                Plugin([$payment_info['payment_float'], $payment_info['payment_subfloat'], $payment_class]);
                $data = array(
                    'body' => $pay_info['data']['subject'],
                    'subject' => $pay_info['data']['subject'],
                    'out_trade_no' => $pay_info['data']['pay_sn'],
                    'total_amount' => $pay_info['data']['api_pay_amount'],
                    'notify_url' => "http://shop.divona.xin/data/plugin/payment/alipay/alipay_wap/notify_url.php",
                    'return_url' => "http://shop.divona.xin/data/plugin/payment/alipay/alipay_wap/return_url.php",
                );
                $pay_obj = new $payment_class();
    ...
    public function aaaOp($param) {
        $inc_file = './api' . DS . 'payment' . DS . 'wxpay' . '.php';
        require_once $inc_file;
        $wxpay = new Wxpay();
        $pre_param = $wxpay->getPrePayOrderWap($param['subject'], $param['pay_sn'], $param['amount']);
