...
class member_chatControl extends mobileMemberControl {
	...
	public function image_sendOp() {
			...
			$upload->set('fprefix', 'sm'.$member_id);
            $upload->set('max_size',C('image_max_filesize')?C('image_max_filesize'):1024);
			$upload->set('allow_type', array('gif', 'jpg', 'jpeg', 'png'));
            $upload->set('default_dir','chat');

            if (!empty($_FILES['_pic']['tmp_name'])){
                $result = $upload->upfile('_pic');
                if ($result){
                    output_data(array('status'=>1,'url'=>UPLOAD_SITE_URL.'/chat/'.$upload->thumb_image));
                }else {
                    output_data(array('status'=>0,'msg'=>$upload->error));
                }
            }
