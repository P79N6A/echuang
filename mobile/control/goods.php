...
class goodsControl extends mobileHomeControl
{
    ...
    public function goods_detailOp()
    {
        ...
        if ($promotion_type == 'groupbuy') {
            $goods_commonid = Model('goods')->getGoodsInfo(array('goods_id'=>$goods_detail['goods_info']['goods_id']), 'goods_commonid');
            $upper_limit = Model('groupbuy')->getUpperLimit($goods_commonid['goods_commonid']);
            if ($upper_limit > 0) {
                //查询该商品其它所有规格goods_id
                $goods_id_arr = Model('goods')->getGoodsIdArr(array('goods_commonid' => $goods_commonid['goods_commonid']));
...
//        $member_info = $member_model->getMemberInfo(array('member_id' => $member_id));
    ...
    public function save_consultOp()
    {
        ...
        //查询会员信息
        $member_id = Model('mb_user_token')->getMbUserTokenInfo(array('token' => $_GET['key']));
        $member_model = Model('member');
        $member_info = $member_model->getMemberInfo(array('member_id' => $member_id['member_id']));
