...
//自动注册会员并登录
				...
				if ($token) {
					...
					redirect(WAP_SITE_URL . '/tmpl/member/member.html?info=hao&username=' . $state_data['username'] . '&key=' . $state_data['key']);
				} else {
					output_error('会员登录失败');
				}
			} else {
				//output_error('微信登录失败'); V5.3
...
//自动注册会员并登录
	...
	public function get_sms_captchaOp() {
		$phone = $_GET['phone'];
		$log_type = $_GET['type']; // 短信类型:1为注册,2为登录,3为找回密码,4支付密码
		if (strlen($phone) == 11) {
			$logic_connect_api = Logic('connect_api');
