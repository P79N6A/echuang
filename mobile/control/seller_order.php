...
class seller_orderControl extends mobileSellerControl {
     ...
     * 虚拟 取消订单
     */
	public function vr_order_cancelOp() {
			$order_id = intval($_POST['order_id']);
			$reason = $_POST['reason'];
			$model_vr_order = Model('vr_order');
			...
			$result = Logic('vr_order')->changeOrderStateCancel($order_info,'seller',$this->seller_info['seller_name'], $reason, true, $cancel_condition);
			if (!$result['state']) {
				output_error($result['msg']);
			}
			output_data('1');
