...
class member_buyControl extends mobileMemberControl {
	...
	public function buy_step1Op() {
		...
		foreach ($result['store_cart_list'] as $key => $value) {
			...
			if (is_array($result['store_voucher_list'][$key]) && count($result['store_voucher_list'][$key]) > 0) {
                ...
                $store_total_list[$key] = $member_discount * $store_total_list[$key];
	...
	public function payOp($pay_sn,$product_num) {
		...
		$pay['member_available_gold_bean'] = 0;
		$pay['member_available_silver_bean'] = 0;
		...
		$pay['member_available_gold_bean'] = Model('member_extend')->getAvailBeanById($this->member_info['member_id']);
		$pay['member_available_silver_bean'] = Model('member_extend')->getAvailSilverBeanById($this->member_info['member_id']);
		$pay['member_paypwd'] = $this->member_info['member_paypwd'] ? true : false;
		$pay['pay_sn'] = $pay_sn;
		$pay['product_num'] = $product_num;
