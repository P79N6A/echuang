...
class rechargeControl extends mobileMemberControl {
	...
	public function recharge_orderOp()
	{
		$pay_sn	= $_POST['paysn'];
	...
	public function recharge_vip1Op(){
		$pdr_amount = abs(floatval($_POST['pdr_amount']));
		$model_setting = Model('setting');
		$list_setting = $model_setting->getListSetting();
	...
	public function recharge_vip2Op(){
		$pdr_amount = abs(floatval($_POST['pdr_amount']));
		$model_setting = Model('setting');
		$list_setting = $model_setting->getListSetting();
	...
	public function viprecharge_orderOp()
	{
		$pay_sn	= $_POST['paysn'];
        if (!preg_match('/^\d{18}$/',$pay_sn)){
           output_error('订单号错误!');
