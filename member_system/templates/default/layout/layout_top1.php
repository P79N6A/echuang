...
<div id="vToolbar" class="nc-appbar">
  <div class="nc-appbar-tabs" id="appBarTabs">
    ...
    <div class="user-login-box" nctype="barLoginBox" style="display:none;"> <i class="arrow"></i> <a href="javascript:void(0);" class="close-a" nctype="close-barLoginBox" title="关闭">X</a>
      <form id="login_form" method="post" action="<?php echo urlMemberSystem('login', 'login'); ?>" onsubmit="ajaxpost('login_form', '', '', 'onerror')">
        ...
        <dl>
          <dt><strong>登录密码</strong><a href="<?php echo urlMemberSystem('login', 'forget_password'); ?>" target="_blank">忘记登录密码？</a></dt>
          <dd>
        ...
        <?php }?>
            <div class="bottom">
              ...
              <a href="<?php echo urlMemberSystem('login', 'register', array('ref_url' => $_GET['ref_url'])); ?>" target="_blank">注册新用户</a>
              <?php if (C('qq_isuse') == 1 || C('sina_isuse') == 1 || C('weixin_isuse') == 1) {?>
              <h4><?php echo $lang['nc_otherlogintip']; ?></h4>
              <?php if (C('weixin_isuse') == 1) {?>
              <a href="javascript:void(0);" onclick="ajax_form('weixin_form', '微信账号登录', '<?php echo urlMemberSystem('connect_wx', 'index'); ?>', 360);" title="微信账号登录" class="mr20">微信</a>
              <?php }?>
...
if ($nav['nav_new_open']) {
		...
		echo ' href="';
		switch ($nav['nav_type']) {
		case '0':echo $nav['nav_url'];
			break;
		case '1':echo urlShop('search', 'index', array('cate_id' => $nav['item_id']));
			break;
		case '2':echo urlMember('article', 'article', array('ac_id' => $nav['item_id']));
			break;
		case '3':echo urlShop('activity', 'index', array('activity_id' => $nav['item_id']));
			break;
		}
		echo '"';
...
<!--              <li><a href="--><?php //echo MEMBER_SITE_URL; ?><!--/index.php?act=member_points" class="arrow">--><?php //echo $lang['hao_points']; ?><!--<i></i></a></li>-->
    ...
    <div class="user-entry">
      ...
      <span style="background-color: red;padding: 5px 5px;"><a href="<?php echo urlMemberSystem('member_info', 'index'); ?>" style="color: white;">进入会员系统</a></span>
      <?php endif?>
      ...
      <?php }?>
      </span><span class="wr"><a href="<?php echo urlMemberSystem('login', 'logout'); ?>"><?php echo $lang['nc_logout']; ?></a></span>
      <?php } else {?>
      <span class="wr"><a class="login" href="<?php echo urlMember('login'); ?>">请<?php echo $lang['nc_login']; ?></a> <a href="<?php echo urlMemberSystem('login', 'register'); ?>"><?php echo $lang['nc_register']; ?></a></span>
      <?php }?>
    ...
    $("#rtoolbar_cart").click(function(){
        if ($("#content-cart").css('right') == '-250px') {
    		...
    		if (!$("#rtoolbar_cartlist").html()) {
    			$("#rtoolbar_cartlist").load('<?php echo SHOP_SITE_URL; ?>/index.php?act=cart&op=ajax_load&type=html');
    		}
        } else {
        	$(".close").click();
        	$(".chat-list").css("display",'none');
