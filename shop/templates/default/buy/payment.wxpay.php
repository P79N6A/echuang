...
function queryOrderState(){
    $.ajax({
        ...
        success: function(result) {
            ...
            if(result.state==1){
                if (result.type == 'r') {
                	top.window.location = '<?php echo SHOP_SITE_URL.'/index.php?act=member_order'?>';
				} else if (result.type == 'pd') {
					top.location.href = '<?php echo SHOP_SITE_URL.'/index.php?act=predeposit&op=index'?>';
                } else {
                	top.window.location = '<?php echo SHOP_SITE_URL.'/index.php?act=member_vr_order&op=index'?>';
                }
            }
        }
    });
