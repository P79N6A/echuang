...
$(function(){
    ...
    if (system.win || system.mac || system.xll||system.ipad) {
        ...
        window.location.href = window.location.href+"/wap";
    }
})
...
<div class="home-sale-layout wrapper">
  <div class="left-layout"><div class="index-sale"><?php echo $output['web_html']['index_sale'];?></div></div>
    ...
    <div class="title">
      <h3>商城快报</h3>
      <!--<h3><?php  //echo  urlShop('goods','index',array('goods_id'=>$v['goods_id']))?></h3>-->
    </div>
     <div class="news">
     <ul>
...
<div class="wrapper mt20">
                <div class="gb-crazy">
                <div class="crazy-hd">
					<span><a href="<?php echo BASE_SITE_URL;?>/index.php?act=show_groupbuy&op=groupbuy_list" target="_blank" title="更多抢购">更多</a></span>
                    <h2>疯狂团购</h2>
                </div>
                <div class="crazy-bd">
                    <ul class="crazy-list clearfix">
						<?php foreach($output['group_list'] as $val) { ?>
                        <li class="mod-shadow-card goods-thumb">
                            <a href="<?php echo urlShop('show_groupbuy','groupbuy_detail',array('group_id'=> $val['groupbuy_id']));?>" target="_blank" class="img"><img src="<?php echo gthumb($val['groupbuy_image1'], 'mid');?>" alt="<?php echo $val['groupbuy_name']; ?>"></a>
                            <p class="price"><?php echo ncPriceFormatForList($val['groupbuy_price']); ?></p>
                            <a href="<?php echo urlShop('show_groupbuy','groupbuy_detail',array('group_id'=> $val['groupbuy_id']));?>" target="_blank" class="name" title="<?php echo $val['groupbuy_name']; ?>"><?php echo $val['groupbuy_name']; ?></a>
                            <div class="lefttime time-remain" count_down="<?php echo $val['end_time']-TIMESTAMP; ?>">
                                <i class="iconfont icon-time"></i>
                                <span>剩余时间：</span>
                                <span class="days" time_id="d">0</span>
                                <em>天</em>
                                <span class="hours" time_id="h">0</span>
                                <em>时</em>
                                <span class="minutes" time_id="m">0</span>
                                <em>分</em>
                                <span class="seconds" time_id="s">0</span>
								<em>秒</em>
                            </div>
                            <a href="<?php echo urlShop('show_groupbuy','groupbuy_detail',array('group_id'=> $val['groupbuy_id']));?>" class="crazy-btn">立即团</a>
                        </li>
						<?php } ?>
					</ul>
                </div>
...
<!--          --><?php //}}?>
<!--        </ul>-->
<!--        <script type="text/javascript">$(document).ready(function() {function statusRunner() {setTimeout(function() {var sl = $('#sl li'), f = $('#sl li:last');f.hide().insertBefore(sl.eq(0)).css('opacity','0.1');f.slideDown(500, function() {	f.animate({	opacity: 1 });});statusRunner();}, 7000);}statusRunner();});</script></div>-->
<!--    </div>-->
<!--  </div>-->
<!--</div>-->
<!--</div>-->
