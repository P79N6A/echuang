...
<div class="wrapper pr">
    ...
    <div class="ncs-detail<?php if ($output['store_info']['is_own_shop']) {
        echo ' ownshop';
    }
    ?>">
        <!-- 焦点图 -->
        ...
        <div class="ncs-goods-summary">
                    ...
                    <dl class="good-price">
                        ...
                        <dd class="price">
                            <?php if (isset($output['goods']['promotion_price']) && !empty($output['goods']['promotion_price'])) {?>
                                <strong><i><?php echo $lang['currency']; ?></i><?php echo ncPriceFormat($output['goods']['promotion_price']); ?></strong><em><?php echo $lang['currency'] . ncPriceFormat($output['goods']['goods_price']); ?></em>
                            <?php } else {?>
          ...
          <dd class="cost-price"><strong><?php echo $lang['currency'] . $output['goods']['goods_price']; ?></strong></dd>
                    ...
                    <dl class="good-price">
                        ...
                        <dd class="price"><strong><?php echo $lang['currency'] . ncPriceFormat((floatval($output['goods']['book_down_payment']) + floatval($output['goods']['book_final_payment']))); ?></strong><em><?php echo $lang['currency'] . $output['goods']['goods_price']; ?></em></dd>
                    </dl>
            ...
            <?php if ($output['goods']['goods_state'] != 10 && $output['goods']['goods_verify'] == 1) {
              ...
              <?php $rule = $output['mansong_info']['rules'][0];
              echo $lang['nc_man'];?>
                                            ...
                                            <div class="sale-rule-content" style="display: none;" nctype="rule-content">
                                                ...
                                                <div class="bottom"><a href="<?php echo urlShop('show_store', 'mansong_goods', array('store_id' => $output['store_info']['store_id'])); ?>" class="url" target="_blank">查看更多供应商“满即送”活动商品</a></div>
                                            </div>
                                        </div>
                                    <?php }?>
                                    <!--E 满就送 -->
                                    ...
                                    <?php if ($output['goods']['jjg_explain']) {
                                        ...
                                        <div class="ncs-jjg"> <span class="sale-name">加价购</span> <span class="sale-rule"><?php echo $output['goods']['jjg_explain']; ?></span> <span nctype="show-rule" class="sale-rule-more"><a href="javascript:void(0);">共<strong><?php echo count($output['goods']['jjg_info']['levels']); ?></strong>项，展开查看<i></i></a></span>
                                            <div class="sale-rule-content" style="display: none;" nctype="rule-content">
                                                ...
                                                <div class="bottom"><a href="<?php echo urlShop('show_store', 'cou_goods', array('store_id' => $output['store_info']['store_id'], 'cou_id' => $output['goods']['jjg_info']['info']['id'])); ?>" class="url" target="_blank">查看更多供应商“加价购”活动商品</a></div>
                                            </div>
                                        </div>
                                    <?php }?>
                                    <!-- E 加价购 -->
                ...
                <dl class="rate">
                    <!-- S 描述相符评分 -->
                    <dd><span>销量<i><?php echo $output['goods']['goods_salenum']; ?></i></span><span><a href="#ncGoodsRate">累评价<i><?php echo $output['goods']['evaluation_count']; ?></i></a></span><span class="star">评分<i class="raty" data-score="<?php echo $output['goods_evaluate_info']['star_average']; ?>"></i></span></dd>
                    <!-- E 描述相符评分 -->
                </dl>
                <!-- E 促销 -->
                ...
                <!--                            <div class="text">-->
                <!--                                <div>-->
                <!--                                    --><?php //echo $output['store_info']['deliver_region'] ? str_replace(' ', '', $output['store_info']['deliver_region'][1]) : '请选择地区' ?>
                <!--                                </div>-->
                <!--                                <b>∨</b> </div>-->
                <!--                            <div class="content">-->
                    ...
                    <?php if (is_array($output['goods']['goods_attr']) || isset($output['goods']['brand_name'])) {
                        ...
                        <ul class="nc-goods-sort" style="<?php if (!$output['goods']['brand_name']) {?>border-bottom:0;<?php }?>">
                            ...
                            <?php if (is_array($output['goods']['goods_attr']) && !empty($output['goods']['goods_attr'])) {
                                ?>
                                <?php foreach ($output['goods']['goods_attr'] as $val) {$val = array_values($val);
...
<script type="text/javascript">
    ...
    function buynow(goods_id,quantity,chain_id,area_id,area_name,area_id_2){
        ...
        <?php if ($output['goods']['goods_offline_time']<time()) {?>
        alert('商品已下架');return;
        <?php }?>
        <?php if ($_SESSION['store_id'] == $output['goods']['store_id']) {?>
        alert('不能购买自己供应商的商品');return;
        <?php }?>
