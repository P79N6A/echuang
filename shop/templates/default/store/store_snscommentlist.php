...
<div class="comment-widget">
        ...
        <form id="commentform_<?php echo $output['stid']; ?>" method="post" action="index.php?act=store_snshome&op=addcomment">
            ...
            <div class="comment-add">
                ...
                <div id="commentseccode<?php echo $output['stid']; ?>" class="seccode">
                    <label for="captcha">
                        <?php echo $lang['nc_checkcode'] . $lang['nc_colon']; ?>
                    </label>
                ...
                <!-- 防止点击Enter键提交 -->
                <div class="act"> <span class="skin-blue"><span class="btn"><a href="javascript:void(0);" nc_type="scommentbtn" data-param='{"txtid":"<?php echo $output['stid']; ?>"}'><?php echo $lang['sns_comment']; ?></a></span></span> <span id="commentcharcount<?php echo $output['stid']; ?>" style="float:right;"></span>
                    <a class="face" nc_type="smiliesbtn" data-param='{"txtid":"comment<?php echo $output[' stid ']; ?>"}' href="javascript:void(0);">
                        <?php echo $lang['sns_smiles']; ?>
                    </a>
                </div>
            </div>
        </form>
    ...
    <ul class="comment-list">
        ...
        <li nc_type="commentrow_<?php echo $v['scomm_id']; ?>"> <a target="_blank" href="index.php?act=member_snshome&mid=<?php echo $v['scomm_memberid']; ?>"><img width="30" height="30" class="clogo" src="<?php if ($v['scomm_memberavatar'] != '') {echo UPLOAD_SITE_URL . '/' . ATTACH_AVATAR . DS . $v['scomm_memberavatar'];} else {echo UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . C('default_user_portrait');}?>" onload="javascript:DrawImage(this,30,30);"></a>
            <div class="detail">
                <p class="words">
                    <a target="_blank" href="index.php?act=member_snshome&mid=<?php echo $v['scomm_memberid']; ?>" class="name">
                        <?php echo $v['scomm_membername']; ?>
                    </a>
                    <?php echo $lang['nc_colon']; ?>
                    <?php echo parsesmiles($v['scomm_content']); ?>
                </p>
                <p class="other">
    ...
    <div class="more">
        <a target="_blank" href="index.php?act=store_snshome&op=straceinfo&st_id=<?php echo $output['stid']; ?>">
            <?php echo $lang['sns_comment_more']; ?>
        </a>
    </div>
...
$(function() {
    $('#commentform_<?php echo $output['
        ...
        messages: {
            commentcontent: {
                required: '<?php echo $lang['
                sns_comment_null ']; ?>',
                maxlength: '<?php echo $lang['
                sns_content_beyond ']; ?>'
            }
        }
    });
    ...
    //分页绑定异步加载事件
    $('#tracereply_<?php echo $output['
        stid ']; ?>').find('.demo').ajaxContent({
        event: 'click',
        loaderType: "img",
        loadingMsg: "<?php echo SHOP_TEMPLATES_URL; ?>/images/transparent.gif",
        target: '#tracereply_<?php echo $output['
        stid ']; ?>'
    });
    <?php }?>
    //评论字符个数动态计算
    $("#content_comment<?php echo $output['stid']; ?>").charCount({
        allowed: 140,
        warning: 10,
        counterContainerID: 'commentcharcount<?php echo $output['
        stid ']; ?>',
        firstCounterText: '<?php echo $lang['
        sns_charcount_tip1 ']; ?>',
        endCounterText: '<?php echo $lang['
        sns_charcount_tip2 ']; ?>',
        errorCounterText: '<?php echo $lang['
        sns_charcount_tip3 ']; ?>'
    });
});
</script>