...
//商品绑定下的规格值
		...
		$gc_info = Model('goods_class')->getGoodsClassInfoById($goodscommon_info['gc_id']);
		$spec_list_arr = explode(',', $gc_info['gc_spec_id']);
		foreach ($spec_list_arr as $key => $value) {
			$spec_info = Model('spec')->getSpecInfo($value);
			$spec_list2[$value]['sp_name'] = $spec_info['sp_name'];
			$spec_value_param['sp_id'] = $value;
			$spec_value_param['gc_id'] = $goodscommon_info['gc_id'];
			$spec_value = Model('spec')->getSpecValueList($spec_value_param, 'sp_value_id,sp_value_name,sp_value_color');
			$spec_list2[$value]['value'] = $spec_value;
		}
		Tpl::output('spec_json', $spec_json);
		Tpl::output('sign_i', count($spec_list2));
		Tpl::output('spec_list', $spec_list2);
		...
		// 取得商品规格的输入值
		$goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,goods_barcode,goods_cost');
		$sp_value = array();
		if (is_array($goods_array) && !empty($goods_array)) {
			...
			foreach ($goods_array as $k => $v) {
				...
				if (!empty($a)) {
					...
					$sp_value['i_' . $id . '|cost'] = $v['goods_cost'];
...
//如果为二级分类，则分组到父级分类下
				...
				} elseif (empty($goods_stcids_tmp[$v])) {
//如果为一级分类而且分组不存在，则建立一个空分组数组
...
//	    var_dump($_POST);die();
        ...
        $result = $logic_goods->updateGoods(
            ...
            try {
                Model('goods')->changeProductState($_POST['commonid'], $_POST['is_bd_product']);

            } catch (Exception $e) {
                showDialog('报单产品状态更改失败', urlShop('store_goods_online', 'index'));
            }
            if ($result['state']) {
                //提交事务
