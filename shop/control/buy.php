...
class buyControl extends BaseBuyControl
{
    ...
    {   
        ...
        // 金豆支付金额
        $pay['payd_bean_amount'] = 0;

        // 还需在线支付金额
        $pay['pay_diff_online_amount'] = 0;

        // 账户可用金豆
        $pay['member_bean'] = 0;

        // 是否显示金豆
        $pay['if_show_bean_show'] = 0;
        // 是否显示银豆
        $pay['if_show_silver_show'] = 0;
        // 是否显示余额
        $pay['if_show_balance_show'] = 1;
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($order_info['order_state'] == ORDER_STATE_NEW) {
                ...
                $pay['payd_bean_amount'] += $order_info['bean_amount'];
            }
        ...
        $pay['pay_diff_online_amount'] = $pay['pay_amount_online'] - $pay['payd_bean_amount'];
        if ($pay['pay_diff_online_amount'] <= 0) {
            showMessage('该订单已支付', 'index.php?act=member_order', 'html', 'error');
        }
        ...
        } elseif ($pay['pay_amount_online'] == $pay['pay_amount']) {
            $pay['order_remind'] = '请您在' . (ORDER_AUTO_CANCEL_TIME * 60) . '分钟内完成支付，逾期订单将自动取消。 ';
        } else {
        ...
        if ($pay['pay_diff_online_amount'] > 0) {
            $avail_bean = Model('member_extend')->getAvailBeanById($_SESSION['member_id']);
            $buyer_info = Model('member_extend')->getMemberExtendInfo(array('member_id' => $_SESSION['member_id']), '*', 'union');
            $pay['member_paypwd'] = $buyer_info['member_paypwd'] ? 1 : 0;
            $balance = Model('member')->getMemberBalanceById($_SESSION['member_id']);
            $pay['member_balance'] = $balance;
            $avail_silver = Model('member_extend')->getAvailSilverBeanById($_SESSION['member_id']);
            // 显示在线支付接口列表
            $model_payment = Model('payment');
            $condition = array();
            //$payment_list = $model_payment->getPaymentOpenList($condition);
            $payment_list = $model_payment->table('payment_list')->where(array('payment_code'=>array('neq','predeposit')))->select();
            if ($avail_bean >= $pay['pay_diff_online_amount'] && isset($payment_list['beanpay'])) {
                $pay['member_bean'] = $avail_bean;
                $pay['if_show_bean_show'] = 1;
            }
            if ($avail_silver >= $pay['pay_diff_online_amount'] && isset($payment_list['silverpay'])) {
                $pay['member_silver'] = $avail_silver;
                $pay['if_show_silver_show'] = 1;
            }
            if (!empty($payment_list)) {
                unset($payment_list['beanpay']);
                unset($payment_list['silverpay']);
				foreach ($payment_list as $k=>$v){
                   if (in_array($payment_list[$k]['payment_code'],array('alipay_scan','wxpay_scan'))){
                       $payment_code[$k] = $payment_list[$k];
                   }
               }
            }
            if (empty($payment_code)) {
                if (!$pay['if_show_bean_show']) {
                    showMessage('暂未找到合适的支付方式', 'index.php?act=member_order', 'html', 'error');
                }
            } else {
                Tpl::output('payment_list', $payment_code);
            }
