...
class searchControl extends BaseHomeControl {
	...
	//str V5.3显示城市
        ...
        if ($city_id != 0) {
            ...
            Tpl::output('now_city_id',$city_id);
            Tpl::output('city_list',$area_list[$city_id]);
        }else{
	...
	//end
	    ...
	    //省 v5.3
            if (intval($_GET['areaid_1']) > 0) {
                $condition['areaid_1'] = intval($_GET['areaid_1']);
            }
            //市
            if (cookie('cityid')) {
                $condition['areaid_2'] = intval(cookie('cityid'));
            }
            //区
            if (intval($_GET['areaid_3']) > 0) {
                $condition['areaid_3'] = intval($_GET['areaid_3']);
            }
	    //end
