...
class Control {
	...
	protected function showLayout() {
		...
		if ($city_id != 0) {
			Tpl::output('now_city', Model('area')->getAreaInfoById($city_id));
		} else {
			Tpl::output('now_city', '全国');
		}
	...
	protected function getMemberAndGradeInfo($is_return = false) {
		...
		if ($_SESSION['member_id']) {
			...
			$member_extend_info = $model_member_extend->getMemberExtendInfo(array('me_member_id' => $_SESSION['member_id']));
			if ($member_info && $member_extend_info) {
				$member_gradeinfo = $model_member->getOneMemberGrade(intval($member_info['member_exppoints']));
				$member_info = array_merge($member_info, $member_gradeinfo, $member_extend_info);
	...
	protected function article() {
		...
		if (!empty($article_array) && is_array($article_array)) {
			foreach ($article_array as $key => $val) {
				if ($val['ac_code'] == 'notice' && !in_array($val['article_position'], array(ARTICLE_POSIT_SHOP, ARTICLE_POSIT_ALL))) {
					continue;
				}
...
class BaseSellerControl extends Control {
	...
	public function __construct() {
		...
		if ($_GET['act'] !== 'seller_login') {
			...
			$url=UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$store_info['store_avatar'];
			Tpl::output('store_image_url',$url);
			$this->checkStoreMsg();
		}
	}
