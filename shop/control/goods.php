...
class goodsControl extends BaseGoodsControl {
	...
	public function indexOp() {
		...
		$size = $store_info['is_own_shop'] ? 6 : 4;
...
//                var_dump($upper_limit);die();
                if ($upper_limit > 0) {
                    ...
                    foreach ($goods_id_arr as $k=>$v){
                        $order_goods_list = $model_order->getOrderGoodsList(array('goods_id' => $v['goods_id'], 'buyer_id' => $buyer_id, 'goods_type' => 2));
                        if ($order_goods_list) {
		...
		$goods_info['goods_offline_time'] = time()+1;
		// dump($goods_info);
		// die;
	...
	public function calcOp() {
		if (!is_numeric($_GET['area_id']) || !is_numeric($_GET['tid'])) {
			return false;
		}
