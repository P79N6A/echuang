...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if ($order_type == 'r') {
		    ...
		    $result = $logic_payment->updateRealOrder($data['out_trade_no'], $pay_type, $order_list, $data["transaction_id"]);
			//反金豆
			Logic('payment')->orderPresentBean($data['out_trade_no']);
			if (!$result['state']) {
		        return false;
		    }
		    $api_pay_amount = 0;
		    if (!empty($order_list)) {
                ...
                $result_payment = $logic_payment->getPaymentInfo('wxpay');
                ...
                $result = $logic_payment->updatePdOrder($data['out_trade_no'],$data["transaction_id"],$payment_info, $order_info);
                if (!$result['state']) {
                        return false;
                }
                $api_pay_amount = 0;
		    ...
		    $order_info = $result['data'];
		    $result = $logic_payment->updateVrOrder($data['out_trade_no'], $pay_type, $order_info, $data["transaction_id"]);
		    if (!$result['state']) {
		        return false;
		    }
		    $api_pay_amount = $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'];
